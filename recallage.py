...
def addNoiseToImage(img, noise, noiseIntensity):
    if noise == 's&p':
        print("Adding S&P Noise: wT = " + str(noiseIntensity['whiteThreshold']) + "; bT = " + str(
            noiseIntensity['blackThreshold']))
        ...
        if noise == 'gaussian':
            print("Adding Gaussian noise: sigma = " + str(noiseIntensity['sigma']) + '; mean = ' + str(
                noiseIntensity['mean']))
...
#readImage("video_travail/test.png")


# main script
def recallage(path_frame_base, path_frame_compar):
    argv = [0, path_frame_base, path_frame_compar]
    timeStart = time.time()
